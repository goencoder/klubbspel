...
func (r *PlayerRepo) FindAllEmaillessPlayersInClub(ctx context.Context, clubID string) ([]*Player, error) {
	...
	if clubID != "" {
		...
		if err == nil {
			...
			if err := debugCursor.All(ctx, &allPlayers); err != nil {
				log.Debug().Err(err).
					Str("club_id", clubID).
					Msg("Failed to read players in club for debugging")
			} else {
				log.Debug().
					Int("player_count", len(allPlayers)).
					Str("club_id", clubID).
					Msg("Found players in club")
				for _, p := range allPlayers {
					log.Debug().
						Str("display_name", p.DisplayName).
						Str("player_id", p.ID.Hex()).
						Bool("is_synthetic_email", IsSyntheticEmail(p.Email)).
						Bool("email_empty", p.Email == "").
						Msg("Player details")
				}
			}
		}
	...
	log.Debug().
		Int("email_less_player_count", len(players)).
		Interface("filter", filter).
		Msg("Found email-less players")
